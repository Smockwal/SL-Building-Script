...
dialog(key user, integer channel) {
    list buttons = ["·Done·", "·Close·", "·Reset·", "-1%", "-5%", "-10%", "+1%", "+5%", "+10%"];
...
default  {
    ...
    listen(integer channel, string name, key id, string message) {
        ...
        float factor;
        if (message == "·Done·")state clean;
        else if (message == "·Close·") {
            llListenControl(gi_hand, 0);
            return ;
        }
        else if (message == "·Reset·") factor = ((float)1) / gf_factor;
        else if (message == "+1%") factor = 1.01;
        else if (message == "+5%") factor = 1.05;
        else if (message == "+10%") factor = 1.1;
        else if (message == "-1%") factor = 0.99;
        else if (message == "-5%") factor = 0.95;
        else if (message == "-10%") factor = 0.9;
